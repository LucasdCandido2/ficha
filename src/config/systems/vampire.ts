...
const vampireConfig: RPGSystem = {
  ...
  fields: [
    ...
    // Grupos de habilidades e conhecimentos
    {
      name: 'talentos',
      label: 'Talentos',
      type: 'group',
    ...
    // Campos adicionais fora dos grupos
  ...
  // Propriedade para cálculos do sistema Vampire
  calculations: {
    // Soma dos atributos por grupo
    totalAtributos: (data) => ({
      fisicos:
        data.atributosFisicos.forca +
        data.atributosFisicos.destreza +
        data.atributosFisicos.vigor,
      sociais:
        data.atributosSociais.carisma +
        data.atributosSociais.manipulacao +
        data.atributosSociais.aparencia,
      mentais:
        data.atributosMentais.percepcao +
        data.atributosMentais.inteligencia +
        data.atributosMentais.raciocinio
    })
    // Você também pode adicionar outros cálculos,
    // como a criação de um "dice pool" que some atributo com uma habilidade específica.
  }
};

export default vampireConfig;